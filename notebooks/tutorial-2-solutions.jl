...
# ╔═╡ 421cc9b5-052c-40d9-a941-5f5aecfaadd6
md"Solution: It is at least what we would expect, since the $H^1$ error is bounded by a term $\mathcal{O}(h^p)$. However, we see that for order 1 it is more than expected. This is not contradicting the theory, which provides an upper bound of the error (it cannot go worse) but it can work better than expected. This is called _super-convergence_."
...
# ╔═╡ 1ae5ca54-a5b4-4931-b414-c4a628f4f34d
md"This is what we expect. Since the function is not smooth, increasing the order does not help in general. We cannot exploit smoothness to get higher convergence rates. The odd-even order pattern is curious, but does not contradict the theory. It seems there is some _super-convergence_ for even orders."
...
# ╔═╡ e9c53750-54ae-42f1-b18a-3dfffafcc9fc
md"Solution: As commented above, increasing $p$ does not improve the theoretical convergence rates, since the solution is not smooth. We see an erratic behavior, no convergence."
...
# ╔═╡ a952a2a7-cd69-48d5-9e21-469be516afdc
md"From theory, we know that $log(e)$ is reduced linearly with $p$. In this case, we can in fact observe this relation (after some preasymptotic regime), with a slope $-0.25$"
...
# ╔═╡ 1d55b48a-e81c-468d-b9d9-00d0a8c74626
md"Solution: Since the solution is very oscillatory, we cannot capture the frequency in the coarser meshes, leading to a _pre-asymptotic_ regime in which we cannot see much improvement. At some point, the approximation space can capture the frequency and we start observing the right order of convergence."
