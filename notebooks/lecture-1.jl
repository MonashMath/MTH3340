...
md"After using the `Gridap` package, we define a function `spectral_solver`. This function requires as input the order of the spectral method we want to use and the exact solution of the problem. Based on the exact solution we want to obtain, we can define the value of the forcing term in the equation and the boundary conditions. This is very common in numerics, the so-called _method of manufactured solutions_. Since we know the exact solution, we can compute errors."
...
md"Now, we can play with both the order or approximation and solution we want to use"
...
md"Calling the `spectral_solver` with these values of $p$ and $u$, we obtain the approximated solution $u_p$ and the $L^2$ and $H^1_0$ errors."

# ╔═╡ 444d62c3-2ba8-4b57-9714-0a8ccc22ea82
uh, el2, eh1 = spectral_solver(p,u)
...
md"Play with the order. What do you see? Does it make sense? Now,you can try other solutions. E.g., choosing $p=3$ and $u = x^2+1$, which is the expected error?"
