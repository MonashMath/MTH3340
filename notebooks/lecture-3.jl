...
md"# Finite element methods for convection in 1D"

# ╔═╡ a4e0c8ad-fbe2-4f7f-aaff-3ba79ee05c08
md"Below, we are defining a finite element method solver for convection-difussion in 1D using Gridap."
...
function finite_element_supg_solver(n,_b)
  ...
  h = 1/n
  τ = 1/(norm(_b)/h + 4/h^2)
  a(u,v) = ∫(v*(b⋅∇(u)))dΩ + ∫(∇(v)⋅∇(u))dΩ + ∫(τ*(b⋅∇(u)-Δ(u))*(b⋅∇(v)))dΩ 
  l(v) = ∫(v*f)dΩ + ∫(τ*(f*((b⋅∇(v)))))dΩ
